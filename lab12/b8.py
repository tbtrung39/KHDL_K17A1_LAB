...
try:
    ...
    try:
        ...
        if thang in [4, 6, 9, 11] and ngay > 30:
            raise ValueError("Tháng này chỉ có 30 ngày.")
        if thang == 2:
            if is_leap_year(nam) and ngay > 29:
                raise ValueError("Tháng 2 trong năm nhuận chỉ có 29 ngày.")
            elif not is_leap_year(nam) and ngay > 28:
                raise ValueError("Tháng 2 trong năm không nhuận chỉ có 28 ngày.")
        if ngay == 1:
            if thang == 1:
                ngay = 31
                thang = 12
